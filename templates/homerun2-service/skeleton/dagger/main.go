// Dagger CI module for ${{ values.fullName }}
//
...
func (m *Dagger) BuildImage(
	...
	src *dagger.Directory,
	// +optional
	// +default="ko.local/${{ values.fullName }}"
	repo string,
	// +optional
	// +default="false"
	push string,
...
`, binName, port, port)
	...
	result := dag.Container().
		...
		WithEnvVariable("REDIS_STREAM", "${{ values.redisStream }}").
...
redis-cli -h redis -p 6379 XADD ${{ values.redisStream }} '*' messageID test-msg-001 > /dev/null
...
func (m *Dagger) SmokeTest(
	ctx context.Context,
	// The base URL of the deployed pitcher (e.g., https://${{ values.fullName }}.example.com)
	endpoint string,
